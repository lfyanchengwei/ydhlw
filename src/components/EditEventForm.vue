<template>
  ...
  export default {
    ...
    async created() {
      ...
      console.log(eventId);
      try {
        const response = await this.$axios.get('/api/events/get',{params:{eventId:eventId}});
        if (response.data.code === 1) {
          this.form = response.data.data;
          this.form.eventId = eventId; // 重置 eventId
        } else {
          this.$message.error(response.data.msg || "获取活动数据失败");
        }
      } catch (error) {
    ...
    methods: {
      async submitForm() {
        this.$refs.eventForm.validate(async (valid) => {
          if (valid) {
            try {
              const response = await this.$axios.post(`/api/events/edit`, this.form);
              if (response.data.code === 1) {
      ...
      cancelEdit() {
        this.$router.push("/eventList"); // 返回活动列表
      },
      async uploadImage(params) {
        ...
        try {
          ...
          if (response.data.code === 1) {
            this.form.picUrl = response.data.data; // 更新图片链接
